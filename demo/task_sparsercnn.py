...
score_threshold = 0.3
max_image_size = 1600
num_proposals = 100  # 默认proposal数量
...
def init_model():
    ...
    # 设置类别元数据
    model.dataset_meta = {'classes': VOC_CLASSES}  # 修改为VOC类别
    
    # 获取proposal数量
    global num_proposals
    num_proposals = model.test_cfg.get('rcnn', {}).get('max_per_img', 100)
    return model
...
class ResultVisualizer:
    ...
    def draw_segmentation(self, img, result, out_path):
        """绘制检测框和掩膜（关键修改：适配Sparse R-CNN输出结构）"""
